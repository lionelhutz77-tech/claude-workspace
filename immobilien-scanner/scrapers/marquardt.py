"""
Scraper für Marquardt Immobilien
https://www.immo-marquardt.de/
"""
...
def scrape_marquardt(postleitzahl: str = "46149") -> List[Dict]:
    """Marquardt Immobilien Oberhausen"""

    logger.info("[SCRAPE] Scraping Marquardt Immobilien...")
    properties = []
    session = requests.Session()
    session.headers.update({"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"})

    try:
        url = "https://www.immo-marquardt.de/"
        response = session.get(url, timeout=10)
        response.raise_for_status()

        soup = BeautifulSoup(response.content, "html.parser")
        listings = soup.select(".property, .listing, .expose, .objekt")
        ...
        for listing in listings[:30]:
            try:
                ...
                addr = listing.select_one("h2, h3, .title, .expose-title")
                adresse = addr.get_text(strip=True) if addr else "???"

                price_elem = listing.select_one(".price, .kosten, [class*='preis']")
                ...
                if link and not link.startswith("http"):
                    link = "https://www.immo-marquardt.de" + link
        ...
        logger.error(f"[ERROR] Fehler Marquardt: {e}")

    logger.info(f"[OK] Marquardt: {len(properties)} Objekte")
    return properties
